...
describe('import preview app contract', () => {
  test('previews imports before applying merge or replace changes', () => {
    ...
    expect(appSource).toContain('className="import-preview-footer"')
    expect(appSource).toContain('导入确认操作')
    expect(appSource).toContain('确认导入方式')
  ...
  test('styles import preview confirmation without using nested cards', () => {
    ...
    expect(css).toContain('.import-preview-footer')
